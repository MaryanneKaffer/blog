    ...
    return (
        <nav className={`h-[55px] ${active ? "text-white" : ""} flex text-md justify-between fixed w-full -translate-x-1/2 left-1/2 top-3 md:px-12 px-6 z-100`}>
            <div className={`items-center h-full duration-500 flex transition-all backdrop-blur-md
                ${active ? "md:w-[3%] w-[17%] rounded-full bg-black dark:bg-default " : "md:w-[50%] w-[40%] rounded-l-full px-6 bg-white/75  dark:bg-default/75 "}`}>
            ...
            <div className={`flex gap-2 dark:bg-default items-center px-6 bg-black h-full duration-500 backdrop-blur-md transition-all
                ${active ? "md:w-[7.5%] w-[42%] rounded-full" : "md:w-[50%] w-[90%] rounded-r-full bg-white/75 dark:bg-default/75"}`}>
                ...
                <button className={`dark:text-black text-white w-[100px] active:scale-[0.9] transition-all 
                    ${!active && "dark:bg-white bg-black p-1 hover:scale-[1.05]"} hover:brightness-125 cursor-pointer rounded-lg md:text-[16px] text-sm hover:scale-[1.1] duration-500`}>
