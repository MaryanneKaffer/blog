...
export default function Latest() {
    ...
    return (
        <section className="w-full flex flex-col gap-7 justify-center overflow-hidden">
            ...
            <div className="flex lg:flex-row flex-col lg:bg-transparent bg-default/55 gap-5 lg:h-[60dvh] h-[70dvh] hover:scale-[1.005] hover:bg-default/45 transition-all duration-500 cursor-pointer p-3 group">
                ...
                <div className="flex flex-col w-full relative lg:h-full h-[58%] relative">
                    <h1 className="text-2xl">{latestPost[language].title}</h1>
                    ...
                    <div className="overflow-hidden lg:h-[90%] h-[85%]">
                        {(latestPost[language].content).map((content) => (
                            <p className="lg:text-lg text-md line-clamp-5">{content}</p>
                        ))}
                    </div>
