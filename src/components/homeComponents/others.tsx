...
export default function Others() {
    const posts = PostsData.filter(post => !post.highlighted).slice(0, 12)

    const { language } = useLanguage()

    return (
        <section className="w-[100%] flex flex-col gap-3 px-3 dark:bg-black/20 bg-default/40 backdrop-blur-md">
            ...
            <div className="flex gap-3 flex-wrap w-full relative lg:justify-normal justify-between">
                {posts.map((post, i) => (
                    <div key={i} className="lg:w-[24.3%] w-[48%] relative group hover:scale-[1.008] lg:bg-transparent bg-default/45 transition-all relative hover:dark:bg-default/40 hover:bg-default/50 backdrop-blur-md lg:p-4 p-2">
                        <img className="object-cover h-[180px]" src={post.fullPicture} />
